...
- **One scene, three fidelity tiers (non-negotiable):** every visitor sees the same golden-hour coast — never a different renderer. **Full** 3D on WebGL2 desktops; **degraded-live** 3D (`quality="calm"`: fewer trees/segments, DPR ≤ 1.25; bloom stays on every tier — it carries the golden-hour glow) on phones and narrow viewports; a **static still image of the same coast** (`StaticCoast`, with DOM buoys over it) when there's no WebGL2 **or** `prefers-reduced-motion`, and as the safety-net if the scene fails to present within ~8s. `RendererStage` owns this decision. The v1 2D `<Ocean>` is **retired**. Recruiters on locked-down laptops still get the coast — as an image.
